...
The Features API exposes REST endpoints for discovering and interacting with registered features remotely.
...
5.  **Run Feature:** `POST|GET /wp/v2/features/{feature-id}/run`
    ...
    *   **Method:** Use `POST` for `tool` features, `GET` for `resource` features (though `POST` often works for resources too if sending context).
    *   **Request Body (JSON for POST):** Contains the context/arguments for the feature.
        ...
        ```
    *   **Query Parameters (for GET resources with params like REST aliases):** Add parameters to the URL if the feature expects them (often derived from `rest_alias` routes). Example: `GET /wp-json/wp/v2/features/resource-post/run?context[id]=1` (although the controller doesn't explicitly parse `context` from query params currently, a direct `call` or custom feature might). REST Aliases map URL params automatically.
